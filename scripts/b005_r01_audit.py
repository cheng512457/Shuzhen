...
files=list(ROOT.rglob(f'B005_R01_Shard{SHARD:02d}_1000_pre_audit.csv'))
if not files: raise RuntimeError(f'Missing B005 R01 shard {SHARD}')
with files[0].open('r',encoding='utf-8-sig',newline='') as f: rows=list(csv.DictReader(f))
s=requests.Session();s.headers.update({'User-Agent':'Shuzhen-B005-DOI-audit/1.0 (mailto:research@example.com)','Accept':'text/html,application/xhtml+xml,application/pdf,*/*;q=0.8'})
...
with (OUT/f'B005_R01_Shard{SHARD:02d}_1000_audited.csv').open('w',encoding='utf-8-sig',newline='') as f:
    w=csv.DictWriter(f,fieldnames=headers);w.writeheader();w.writerows(res)
summary={'stage':'B005-R01-audit','shard':SHARD,'input_rows':len(rows),'output_rows':len(res),'unique_dois':len({r.get('doi') for r in res}),'audit_result_counts':dict(Counter(r.get('link_audit_result') for r in res)),'http_status_counts':dict(Counter(str(r.get('link_http_status')) for r in res))}
(OUT/f'B005_R01_Shard{SHARD:02d}_audit_summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
