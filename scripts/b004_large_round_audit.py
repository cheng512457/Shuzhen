...
files = list(ROOT.rglob(f'B004_{ROUND_CODE}_Shard{SHARD:02d}_{SHARD_SIZE}_pre_audit.csv'))
if not files:
    raise RuntimeError(f'Missing {ROUND_CODE} shard {SHARD:02d} pre-audit file')
source = files[0]
...
session.headers.update({
    'User-Agent':'Shuzhen-B004-large-round-DOI-audit/1.0 (mailto:research@example.com)',
...
with ThreadPoolExecutor(max_workers=24) as ex:
    ...
    for idx, fut in enumerate(as_completed(futures), 1):
        ...
        if idx % 100 == 0:
            print('AUDIT_PROGRESS', ROUND_CODE, SHARD, idx, dict(Counter(r['link_audit_result'] for r in results)), flush=True)

results.sort(key=lambda r: r.get('B004_ID') or '')
headers = list(results[0].keys()) if results else []
with (OUT/f'B004_{ROUND_CODE}_Shard{SHARD:02d}_{SHARD_SIZE}_audited.csv').open('w', encoding='utf-8-sig', newline='') as f:
    w = csv.DictWriter(f, fieldnames=headers)
    w.writeheader(); w.writerows(results)

summary = {
    'stage':'S4-large-audit','round_code':ROUND_CODE,'shard':SHARD,
    'input_rows':len(rows),'output_rows':len(results),
...
(OUT/f'B004_{ROUND_CODE}_Shard{SHARD:02d}_audit_summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
print(json.dumps(summary,ensure_ascii=False),flush=True)
if len(results) != SHARD_SIZE or summary['unique_dois'] != SHARD_SIZE:
    raise SystemExit(2)
