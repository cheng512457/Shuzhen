...
files = list(ROOT.rglob('B004_B005_B006_B007_B008_285204_cumulative_master.csv'))
if len(files) != 1:
...
if len(records) != 285204 or len(excluded) != 285204:
    raise RuntimeError(f'Unexpected cumulative registry size rows={len(records)} dois={len(excluded)}')
(OUT / 'B004_B005_B006_B007_B008_285204_excluded_dois.txt').write_text(
...
def seed_score(row):
    score = priority_weight.get(row.get('download_priority'), 0) + relevance_weight.get(row.get('relevance'), 0)
    ...
    score += 0.4 if str(row.get('metadata_status') or '').startswith(('V2', 'complete')) else 0.0
...
for row in records:
    ...
    if k in valid_k and row.get('relevance') in {'A', 'B'}:
        by_k[k].append(row)
headers = ['doi', 'title', 'year', 'journal', 'first_author', 'abstract', 'cited_by_count',
           'K_primary', 'relevance', 'download_priority', 'openalex_id', 'seed_score']
seed_counts = {}
for k in [f'K{i:02d}' for i in range(1, 17)]:
    ranked = sorted(
        by_k[k],
        key=lambda r: (seed_score(r), number(r.get('cited_by_count')), number(r.get('year'))),
        reverse=True,
    )
    selected = ranked[:600]
    seed_counts[k] = len(selected)
    with (OUT / f'B009_E1_seeds_{k}.csv').open('w', encoding='utf-8-sig', newline='') as f:
...
summary = {
    ...
    'quality_gate': {
        'excluded_unique_dois': 285204,
        'each_K_seed_min': 450,
        'total_seeds_min': 8000,
    },
}
(OUT / 'prepare_summary.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
print(json.dumps(summary, ensure_ascii=False), flush=True)
if len(excluded) != 285204 or min(seed_counts.values()) < 450 or sum(seed_counts.values()) < 8000:
    raise SystemExit(2)
