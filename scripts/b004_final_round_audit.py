...
files = list(ROOT.rglob(f'B004_{ROUND_CODE}_Shard{SHARD:02d}_pre_audit.csv'))
if len(files) != 1:
    raise RuntimeError(f'Expected one {ROUND_CODE} shard {SHARD:02d}, found {len(files)}')
source = files[0]
with source.open('r', encoding='utf-8-sig', newline='') as f:
    rows = list(csv.DictReader(f))
if not rows:
    raise RuntimeError('Empty final-round audit shard')

session = requests.Session()
session.headers.update({
    'User-Agent':'Shuzhen-B004-final-round-DOI-audit/1.0 (mailto:research@example.com)',
...
with (OUT/f'B004_{ROUND_CODE}_Shard{SHARD:02d}_audited.csv').open('w', encoding='utf-8-sig', newline='') as f:
    w = csv.DictWriter(f, fieldnames=headers)
    w.writeheader(); w.writerows(results)
summary = {
    'stage':'S4-final-audit','round_code':ROUND_CODE,'shard':SHARD,
