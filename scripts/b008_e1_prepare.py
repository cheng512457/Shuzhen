...
files = list(ROOT.rglob('B004_B005_B006_B007_266798_cumulative_master.csv'))
if len(files) != 1:
...
def seed_score(row):
    ...
    s+=0.4 if str(row.get('metadata_status') or '').startswith(('V2','complete')) else 0.0
    return round(s,4)
...
for k in [f'K{i:02d}' for i in range(1,17)]:
    ...
    with (OUT/f'B008_E1_seeds_{k}.csv').open('w',encoding='utf-8-sig',newline='') as f:
...
summary={'stage':'B008-E1-prepare','source_file':str(source),'excluded_unique_dois':len(excluded),'source_K_counts':dict(counts),'seed_counts':seed_counts,'total_seeds':sum(seed_counts.values()),'quality_gate':{'excluded_unique_dois':266798,'each_K_seed_min':450,'total_seeds_min':8000}}
(OUT/'prepare_summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
print(json.dumps(summary,ensure_ascii=False),flush=True)
if len(excluded)!=266798 or min(seed_counts.values())<450 or sum(seed_counts.values())<8000:
    raise SystemExit(2)
