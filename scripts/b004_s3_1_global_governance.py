...
def record_score(row):
    ...
    if dtype not in PREPRINT_TYPES:
        score += 5
    stage = row.get("source_stage") or ""
    score += {"S2.2": 3, "S2.1": 2, "S2.3": 1}.get(stage, 0)
    return score
...
summary_paths = {
    "S2.1": list(ROOT.rglob("s2_1/**/run_summary.json")),
    "S2.2": list(ROOT.rglob("s2_2/**/run_summary.json")),
    "S2.3": list(ROOT.rglob("s2_3/**/run_summary.json")),
}
prior_summaries = {}
for stage, paths in summary_paths.items():
    if paths:
        try:
...
cur.execute("""CREATE TABLE raw_record (
...
cur.execute("CREATE INDEX idx_raw_doi ON raw_record(doi)")
...
def insert_row(stage, row):
    ...
    if stage == "S2.1":
        ...
        external_ids = row.get("openalex_id") or ""
    cur.execute("INSERT INTO raw_record VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (
...
cur.execute("""CREATE TABLE doi_registry (
...
# Stream DOI groups in sorted order.
...
def flush_group(group):
    ...
    if len(source_stages) >= 2 and consistency >= 0.80:
        ...
        if len(conflicts) < 10000:
            conflicts.append({"doi":doi,"source_stages":"; ".join(source_stages),"title_consistency":round(consistency,4),"titles":" || ".join(sorted(set(r['title'] for r in rows if r['title'])))})
    ...
    if RETRACTION_RE.search(text):
        ...
        integrity = "normal"
    cur.execute("INSERT INTO doi_registry VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (
...
# Exact normalized-title version relations across different DOIs.
cur.execute("CREATE INDEX idx_registry_title_norm ON doi_registry(title_norm)")
version_groups = 0
version_alternates = 0
version_relations = []
for title_norm, count in cur.execute("SELECT title_norm,COUNT(*) FROM doi_registry WHERE LENGTH(title_norm)>=40 GROUP BY title_norm HAVING COUNT(*)>1"):
    items = list(cur.execute("SELECT doi,document_type,year,journal,title,metadata_status,cited_by_count FROM doi_registry WHERE title_norm=?", (title_norm,)))
    if len(items) < 2:
        continue
    version_groups += 1
    ...
    cur.execute("UPDATE doi_registry SET version_group_id=?,canonical_in_version_group=1,alternate_version_dois=? WHERE doi=?", (gid, "; ".join(d for d in all_dois if d != canonical_doi), canonical_doi))
    for item in items:
        if item[0] == canonical_doi:
            continue
        version_alternates += 1
        cur.execute("UPDATE doi_registry SET version_group_id=?,canonical_in_version_group=0,alternate_version_dois=? WHERE doi=?", (gid, canonical_doi, item[0]))
        version_relations.append({"version_group_id":gid,"canonical_doi":canonical_doi,"alternate_doi":item[0],"normalized_title":title_norm,"canonical_type":canonical[1],"alternate_type":item[1]})
conn.commit()
...
with (OUT / "B004_S3_1_all_unique_doi_registry.csv").open("w", encoding="utf-8-sig", newline="") as f:
    ...
    for row in cur.execute(select_registry + " ORDER BY doi"):
        w.writerow(row)
...
with (OUT / "B004_S3_1_canonical_candidate_pool.csv").open("w", encoding="utf-8-sig", newline="") as f:
    w = csv.writer(f); w.writerow(registry_headers)
    for row in cur.execute(select_registry + candidate_where + " ORDER BY source_record_count DESC,cited_by_count DESC"):
        w.writerow(row)
...
# Counts and quality gate.
all_unique = cur.execute("SELECT COUNT(*) FROM doi_registry").fetchone()[0]
canonical_candidates = cur.execute("SELECT COUNT(*) FROM doi_registry" + candidate_where).fetchone()[0]
status_counts = dict(cur.execute("SELECT metadata_status,COUNT(*) FROM doi_registry GROUP BY metadata_status"))
integrity_counts = dict(cur.execute("SELECT integrity_status,COUNT(*) FROM doi_registry GROUP BY integrity_status"))
source_stage_counts = dict(cur.execute("SELECT source_stages,COUNT(*) FROM doi_registry GROUP BY source_stages"))
missing_title = cur.execute("SELECT COUNT(*) FROM doi_registry WHERE title='' OR title IS NULL").fetchone()[0]
missing_journal = cur.execute("SELECT COUNT(*) FROM doi_registry WHERE journal='' OR journal IS NULL").fetchone()[0]
invalid_total = sum(v for k,v in input_counts.items() if k.endswith("invalid_doi_rows"))
input_total = sum(v for k,v in input_counts.items() if k.endswith("_rows") and not k.endswith("valid_doi_rows") and not k.endswith("invalid_doi_rows"))
...
summary = {
    ...
    "input_total_rows":input_total,
    "invalid_doi_rows":invalid_total,
    "invalid_doi_ratio":round(invalid_ratio,6),
    "all_normalized_unique_dois":all_unique,
    "canonical_candidate_pool":canonical_candidates,
    ...
    "metadata_title_conflicts":len(conflicts),
    "exact_title_version_groups":version_groups,
    "alternate_versions_removed_from_canonical_pool":version_alternates,
    "success_gate":{
        "all_normalized_unique_dois_min":350000,
        "canonical_candidate_pool_min":350000,
        "invalid_doi_ratio_max":0.05,
        "missing_title_ratio_max":0.01,
...
report = [
    "# B004 Stage S3.1 Report","",
    f"- Status: **{summary['status']}**",
    f"- Input rows: {input_total:,}",
    f"- Normalized unique DOIs: {all_unique:,}",
    f"- Canonical candidate pool: {canonical_candidates:,}",
    f"- Invalid DOI rows: {invalid_total:,} ({invalid_ratio:.2%})",
    f"- Metadata status: {status_counts}",
    f"- Integrity status: {integrity_counts}",
